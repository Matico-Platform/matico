import {AggregateStep, AggregationType, DatasetTransform as DatasetTransformBase, DatasetTransformStep, FilterStep, JoinStep} from '@maticoapp/matico_types/spec'
...
export interface DatasetTransformInterface extends DatasetTransformBase{
  ...
  applyStep(step:DatasetTransformStep, table: ColumnTable, datasets:Record<string,LocalDataset>,stepNo : number) : ColumnTable
  applyJoin(step:JoinStep, table: ColumnTable, datasets:Record<string,LocalDataset>,stepNo : number) : ColumnTable
  applyFilter(step:FilterStep, table: ColumnTable, datasets:Record<string,LocalDataset>,stepNo : number) : ColumnTable
...
export class DatasetTransform implements DatasetTransformInterface{
  ...
  applyJoin(step: JoinStep, table: ColumnTable, datasets: Record<string,LocalDataset>, stepNo:number): ColumnTable {
    ...
    if(!datasets[step.otherSourceId]){
      throw new TransformStepError(this.id, stepNo, "Could not find join dataset")
    }

    if(step.joinColumnsLeft.length=== 0 || step.joinColumnsRight.length===0){
      throw new TransformStepError(this.id, stepNo, "Add at least one join column")
    }
    ...
    switch(step.joinType){
      case "left":
        ...
        break
      case "right":
        workingTable = workingTable.join_right( otherSource, joinColumns)
        break
      case "inner":
  ...
  applyFilter(step: FilterStep, table: ColumnTable, datasets: Record<string,LocalDataset>, stepNo:number): ColumnTable {
    try{
      ...
      throw new TransformStepError(this.id, stepNo, `Failed to apply filters ${err}`)
    }
  }
  ...
  applyStep(step:DatasetTransformStep, table:ColumnTable, datasets:Record<string,LocalDataset>, stepNo:number){
    switch (step.type){
      case "join":
