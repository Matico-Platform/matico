...
import { DatasetSummary } from "./NewDatasetService"
...
export class LocalDataset implements Dataset {
  ...
  summary() {
    let datasetSummary: DatasetSummary = {
      name: this.name,
      columns: this._columns,
      shape: [this._data.numRows(), this._columns.length]
    }
  ...
  async query(query: DatasetQuery): Promise<QueryResult> {
    const { column, parameters, metric } = query
    try {
      const result = await (async () => {
        switch (metric) {
          case "max":
            return this.getColumnMax(column)
          case "min":
            return this.getColumnMin(column);
          case "extent":
            return this.getColumnExtent(column);
          case "equalInterval":
            return this.getEqualIntervalBins(
              column,
              parameters?.bins
            );
          case "quantile":
            return this.getQuantileBins(column, parameters.bins);
          case "histogram":
            return this.getColumnHistogram(
              column,
              parameters?.bins,
            );
          case "categoryCounts":
            return this.getCategoryCounts(column);
          case "categories":
          ...
          default:
            throw Error(`Metric ${query.metric} not found`);
        }
      })();
      return {
        query,
  ...
  getCategories(
    ...
    let counts = applyFilters(this._data, filters)
      .groupby(column)
      .count()
      .orderby(desc("count"))
      .array(column) as Array<string | number>;

    if (noCategories !== undefined) {
      counts = counts.slice(0, noCategories)
    }
  ...
  async getColumnHistogram(
    ...
    console.log("raw result ", result, noBins)
