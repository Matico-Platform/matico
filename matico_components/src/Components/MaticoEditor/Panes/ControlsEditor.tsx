      ...
      <Dialog>
        <Content>
          <Flex direction="column" gap="size-150">
            ...
            label="name" value={selectProps.name} />
            <TextField
            ...
            <Flex direction="column" gap="size-150">
              ...
              >
                {avaliableControls.map((control) => (
                  <ActionButton
                    onPress={() => {
                      attemptToAddControl(control.name, close);
                    }}
                  >
...
export const ControlsPaneEditor: React.FC<PaneEditorProps> = ({ editPath }) => {
  ...
  const controlsPane = _.get(spec, editPath);
  ...
  const addControl = (controlName: string, controlType: "Range" | "Select") => {
    const newControl = { ...DefaultForControl[controlType], name: controlName };
      ...
      <Well>
        <Heading>
          <Flex direction="row" justifyContent="space-between" alignItems="end">
            <Text>Controls</Text>
            <AddControlModal onAddControl={addControl} />
          </Flex>
        </Heading>
              ...
              entryName={
                controlType === "Range" ? (
                  <EditRangeModal
                    ...
                    onUpdate={(update) =>
                      updateControlAtIndex(update, index, controlType)
                    }
                  />
                ) : (
                  <EditSelectModal
