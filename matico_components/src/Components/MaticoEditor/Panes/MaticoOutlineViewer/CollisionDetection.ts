...
const generateCollisionDetection = (
    intersectionFunction: typeof rectIntersection,
    proximityFunction: typeof closestCenter
): CollisionDetection => {
    return (args) => {
        ...
        const intersections = intersectionFunction(args).filter(
            (intersected) => {
        ...
        if (active?.data?.current?.type === "page") {
            const pages = intersections.filter(
                (intersected) =>
                    intersected?.data?.droppableContainer?.data?.current
                        ?.type === "page"
            );
        ...
        if (!siblings.length) {
            ...
            const closestSibling = proximityFunction({
                ...args,
                droppableContainers: siblings.map(
                    (item) => item.data.droppableContainer
                )
            });
            const closestParent = proximityFunction({
                ...args,
                droppableContainers: parents.map(
                    (item) => item.data.droppableContainer
                )
            ...
            if (
                parents?.length === 1 &&
                parents[0]?.data?.droppableContainer?.data?.current?.depth === 0
            ) {
                return closestSibling;
            }
            const parentTop =
                closestParent[0]?.data?.droppableContainer?.rect?.current?.top;
            const parentBottom =
                closestParent[0]?.data?.droppableContainer?.rect?.current
                    ?.bottom;
...
export const outlineCollisionDetection: CollisionDetection =
    generateCollisionDetection(rectIntersection, closestCorners);
export const layoutCollisionDetection: CollisionDetection =
    generateCollisionDetection(pointerWithin, closestCenter);
