...
const LinearContainer: React.FC = ({ children, ...rest }) => {
    return <View overflow="hidden auto" width="100%" height="100%" {...rest}>{children}</View>;
}

const GridContainer: React.FC = ({ children, ...rest }) => {
    return <Grid width="100%" height="100%" {...rest}>{children}</Grid>
}

const FreeContainer: React.FC = ({ children, ...rest }) => {
    return <View position="relative" width="100%" height="100%" {...rest}>{children}</View>;
}
...
export const MaticoContainerPane: React.FC<MaticoContainerPaneInterface> = ({
  ...
  return (
    <View 
      ...
      >
      <ControlActionBar editPath={`${editPath}.Container`} editType={"Container"} />
