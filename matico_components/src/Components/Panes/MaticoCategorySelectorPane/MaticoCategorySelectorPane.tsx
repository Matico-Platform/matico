...
export const MaticoCategorySelector: React.FC<MaticoCateogrySelectorInterface> = ({
  dataset,
  column = "",
  id,
  name
  ...
  const filteredOptions = uniqueValueRequest?.result?.filter((c: any) => search && search.length ? c.name.includes(search) : true)

  const selectAll = () => {
    setSelectedValues({ type: "category", value: { oneOf: uniqueValueRequest?.result?.map((v: any) => v.name).filter(f => f !== null || f !== undefined), notOneOf: [] } });
  }

  const selectNone = () => {
    setSelectedValues({ type: "category", value: { oneOf: [], notOneOf: [] } });
  }
  ...
  return (
    ...
    >
      {datasetReady ? (
        <View width={"100%"} height={"100%"} paddingX='size-200' UNSAFE_style={{ boxSizing: "border-box" }}>
          <Flex direction='column' width="100%" height="100%">
            ...
            {uniqueValueRequest &&
              <>
                <View flex={1} overflow={{ y: "auto" }} id="container">
                  <CheckboxGroup
                    label={column}
                    value={selectedValues.value.oneOf}
                    onChange={(keys) => {
                      setSelectedValues({ type: "category", value: { oneOf: keys, notOneOf: [] } });
                    }}
                    flex={1}
                  >
                    <Grid
                ...
                <Flex direction='row'>
                  ...
                  <Button variant="secondary" onPress={selectAll}>Select All</Button>
                  <Button variant="secondary" onPress={selectNone}>Select None</Button>
                </Flex>
              </>
            }
          </Flex>
