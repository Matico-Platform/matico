...
import { PolygonLayer, GeoJsonLayer } from "@deck.gl/layers/typed";
...
const GeoBoundsSelectorInner: React.FC<GeoBoundsSelectorInnerProps> = ({
    ...
    const handleToggleDrag = ({
        ...
        if (isDragging) {
            ...
            // start drag
            setIsDragging((prev) => !prev);
            setHighlightBounds([coordinate, coordinate]);
        }
    };

    const handleDrag = ({ coordinate }: { coordinate: [number, number] }) =>
        isDragging && setHighlightBounds((prev) => [prev[0], coordinate]);
