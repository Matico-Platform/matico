...
export const useRegisterDatasets = () => {
    ...
    const { clearErrors, throwError } = useErrorsOfType(
        MaticoErrorType.DatasetError
    );
    ...
    const normalizedDatasetSpec = useNormalizedSpecSelector(
        (spec) => spec?.datasets
    );
    const normalizedDatasetTransforms = useNormalizedSpecSelector(
        (spec) => spec?.datasetTransforms
    );
    ...
    useEffect(() => {
        ...
        previousDatasetSpec.current.forEach((datasetDetails: DatasetSpec) => {
            const currentSpec = normalizedDatasetSpec.find(
                (d) => d.name === datasetDetails.name
            );
            if (!currentSpec) {
                dispatch(
                    unregisterDataset({
    ...
    useEffect(() => {
        ...
        // If the full normalized spec is the same as the previous one, simply return
        if (
            _.isEqual(normalizedDatasetTransforms, previousTransforms.current)
        ) {
            return;
        }
        normalizedDatasetTransforms.forEach(
            (transformDetails: DatasetTransform) => {
                clearErrors();
