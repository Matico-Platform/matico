...
const loadPageOfDatasets = async (
    portal: string,
    page: number,
    perPage: number
) => {
    const url = `https://api.us.socrata.com/api/catalog/v1?domains=${portal}&search_context=${portal}&limit=${perPage}&offset=${
        perPage * page
    }`;
...
const loadDataasets = async (portal: PortalInfo, onProgress?: progressFunc) => {
    ...
    for (let page = 0; page < pages; page++) {
        let pagedDatasets = await loadPageOfDatasets(
            portal.domain,
            page,
            perPage
        );
