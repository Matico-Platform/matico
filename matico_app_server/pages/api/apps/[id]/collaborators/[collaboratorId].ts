...
export default async function handler(
  ...
  if (!app) {
    res.status(404).json({ error: "Failed to find app" });
  }

  if (!userHasManage(app, user)) {
    res
      .status(401)
      .json({ error: "You dont have permission to manage this app" });
  }

  if (req.method === "DELETE") {
    let id = req.query.id;
    let result = await prisma.collaborator.delete({
      where: { id: req.query.collaboratorId },
    });
