...
export const SyncHistory: React.FC<SyncHistoryProps> = ({ datasetId }) => {
  ...
  const nextScheduled = syncHistory
    ? syncHistory.find((sh: any) => ["Pending", "InProgress"].includes( sh.status) )
    : null;
  const historical = syncHistory
    ? syncHistory.filter((sh: any) => !["Pending", "InProgress"].includes(sh.status))
    : [];
  ...
  return (
    <View>
      <ListBox >
        <Section title={"Next scheduled sync"} items={nextScheduled ? [nextScheduled] : []}>
          {(item) => (
            <Item key="pending">
              { item.status==='inProgress'  ? "Runnning now"  : formatDistanceToNow(
                parseJSON(nextScheduled.scheduled_for),
