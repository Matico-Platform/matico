...
#[get("")]
async fn list_analysis() -> Result<HttpResponse, ServiceError> {
    ...
    let paths = glob(&search_path).expect("The compute directory to exist");
    let computes: Vec<ComputeDetails> = paths
        .filter_map(|path| {
