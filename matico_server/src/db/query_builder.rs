use crate::{
    errors::ServiceError,
    models::{
        datasets::Extent, Api, Column as DatasetColumn, Dataset, StatParams, StatResults, User,
    },
...
pub trait QueryBuilder<T> {
    ...
    async fn get_extent(&self, db: &T, geom_col: &str) -> Result<Extent, ServiceError>;

    /// Run the column query associated with this request and return column definitions
    async fn get_columns(&self, db: &T) -> Result<Vec<DatasetColumn>, ServiceError>;
