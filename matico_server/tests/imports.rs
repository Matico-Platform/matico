...
async fn upload_json_dataset_logged_in_with_bad_metadata() {
    ...
    assert!(
        ...
        "Tried to upload geojson with bad metadata and it did not fail"
    );
}
...
async fn upload_json_dataset_logged_in_good_metadata() {
    ...
    assert!(
        file_upload_result.status().is_success(),
        "Tried to upload geojson with good metadata and it failed"
    );

    let dataset_body: Value = file_upload_result.json().await.unwrap();

    assert_eq!(
        dataset_body["owner_id"],
        user.user.id.to_string(),
        "Owner id doesn't match the user who uploaded the dataset"
    );
    assert_eq!(
        dataset_body["name"], "test dataset",
        "The name of the dataset should be the one we specified"
    );
    assert_eq!(
        dataset_body["table_name"], "test_dataset",
        "The name of the dataset should be the one we specified"
    );
    assert_eq!(
        dataset_body["description"], "Some dataset that we are testing with",
        "The description of the dataset should be the one we specified"
    );
    assert_eq!(
        dataset_body["sync_dataset"], false,
        "This should not be a sync dataset"
    );
    assert_eq!(
        dataset_body["public"], false,
        "This dataset should not be public"
    );
    assert_eq!(
        dataset_body["geom_col"], "wkb_geometry",
        "The geom_col should be correct"
    );
    ...
    assert!(
        result.is_ok(),
        "We should get back the correct column types : {:#?}",
        result
    );
    ...
    assert_eq!(
        String::from("ST_Point"),
        result.unwrap(),
        "Should get a Point geometry back"
    );
}
...
async fn upload_csv_dataset_logged_in_good_metadata_lat_lng() {
    ...
    let file_upload_result = upload_file(
        "resources/test/csv_data_with_lat_lng.csv",
        &url,
        metadata.to_string(),
        Some(&user.token),
    )
    ...
    assert!(
        file_upload_result.status().is_success(),
        "Tried to upload csv with good metadata and it failed {:#?}",
        file_upload_result.text().await.unwrap()
    );
