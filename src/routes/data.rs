use actix_web::{get,web,put,HttpResponse};
use crate::db::DbPool;
...
#[get("{dataset_id}/data")]
async fn get_data(db: web::Data<DbPool>, 
...
async fn get_feature(db: web::Data<DbPool>, 
    ...
    let query = format!("Select * from {table} where {id_col} ={feature_id}",
table=table, id_col = id_col, feature_id = feature_id); 
...
async fn update_feature(db:web::Data<DbPool>,
    web::Path((dataset_id, feature_id)): web::Path<(Uuid,String)>,
    web::Json(update) : web::Json<serde_json::Value>
)->Result<HttpResponse,ServiceError>{
    let dataset = Dataset::find(db.get_ref(), dataset_id)?;
    let result = dataset.update_feature(db.get_ref(),feature_id,update).await?;
