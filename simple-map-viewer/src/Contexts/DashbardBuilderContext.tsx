...
const addLayerToDash = (dashboard: Dashboard| null, layer:Layer)=>{
    if(dashboard){
        const newDash = {...dashboard}
        newDash.map_style.layers = [...newDash.map_style.layers, layer]
        return newDash
    }else{
        return null
    }
...
const updateLayerStyleOnDash = (dashboard:Dashboard| null, update: {name: string, style: LayerStyle})=>{
    if(dashboard){
        const newDash = {...dashboard}
...
const updateBaseMap = (dashboard: Dashboard | null, baseMap: BaseMap)=>{
    if(dashboard){
        const newDash = {...dashboard}
...
function reducer(state: DashboardBuilderState, action: DashboardBuilderAction) {
    console.info("Running action ",action);
    switch (action.type) {
        case ActionType.ADD_LAYER:
            return {...state, dashboard : addLayerToDash(state.dashboard, action.payload)}
        ...
        case ActionType.SET_BASEMAP:
            return {...state, dashboard: updateBaseMap(state.dashboard,action.payload)}
        ...
        case ActionType.UPDATE_LAYER_STYLE:
            return {...state, dashboard: updateLayerStyleOnDash(state.dashboard, action.payload)}
        default:
            return state
    }
}
...
export const DashboardBuilderProvider : React.FC<{dashboard_id:string|null}> = ({children, dashboard_id})=>{
    ...
    useEffect(()=>{
        if(dashboard){
            ...
            updateDashboard(dashboard.id, update).then(result=>{
                dispatch({
                    type: ActionType.SET_DASHBOARD,
                    payload: result.data 
                })
    ...
    useEffect(()=>{
        if(dashboard_id){
            ...
            getDashboard(dashboard_id).then(dashboardResp=>{
                dispatch({
                    type: ActionType.SET_DASHBOARD,
