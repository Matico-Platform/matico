...
function reducer(state: UserState, action: UserAction): UserState{
    switch(action.type){
        ...
        case ActionType.SIGNUP_FAILED:
            return{...state, attempting_signup:false,errors:[action.payload]}
        case ActionType.SIGNED_OUT:
            return {...state, user:null}
        case ActionType.SIGNUP_SUCCESSFUL:
            return{...state, user:action.payload, errors:[], attempting_signup:false}
        default:
            return state
    }
}
...
export const useUser = ()=>{
    ...
    const trySignup = (username:string, password:string, email:string)=>{
        ...
        signup(username,password,email).then(result=>{
            localStorage.setItem('token',result.data.token)
            dispatch({
                type: ActionType.SIGNUP_SUCCESSFUL,
                payload: result.data.user
